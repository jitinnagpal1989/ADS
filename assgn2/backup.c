...
int main(int argc, const char *argv[]){
	...
	if( strcmp(order,"--first-fit") != 0 && strcmp(order,"--best-fit") != 0 && strcmp(order,"--worst-fit") != 0 )
   {
	...
	const char *student_file_name = argv[2];

	if ( ( output_student_file = fopen( student_file_name, "r+b" ) ) == NULL ) {
	 output_student_file = fopen( "student.db", "w+b" ); 
	 output_avail_file = fopen("avail.bin","w+b");
	 output_index_file = fopen("index.bin","w+b");
	} 

	if((output_index_file = fopen("index.bin","r+b")) == NULL){
		printf("ERROR: Index file doesn't exist\n");
      	exit(-1);
	}
	if((output_avail_file = fopen("avail.bin","r+b")) == NULL){
	...
	while(1){
		...
		if(str_token!=NULL){
			...
			exit(0);
				...
				int size = strlen(str_token) + sizeof(int)-1;
				str_token = strtok(NULL," \n");
				int key=0;
				if(str_token!=NULL){
					key = atoi(str_token);
...
void display(){
	...
	printf("Total space occupied by holes=%d\n",total_holes_space);
...
void add_record(int key, char* lastname, char* firstname, char* major, char* key_string){
	...
	char* record = (char*)malloc(strlen(lastname)+strlen(firstname) + strlen(major) + strlen(key_string) +1);
	...
	int buffer[1] = {size};
	fwrite(buffer,sizeof(int),1,output_student_file);
	fwrite(record,sizeof(char),size,output_student_file);
}
...
void del_record(int key){
	...
	if(offset==-1){
		printf("Record doesn't exist, exiting..\n");
		exit(-1);
	}else{
